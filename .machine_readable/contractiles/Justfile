...
REPO := "krl"
version := "0.1.0"
tier := "library"  # 1 | 2 | infrastructure
...
verify-template:
    ...
    # Check for double-brace placeholder patterns (using hex to avoid self-match)
    LB=$(printf '\x7b\x7b')
    RB=$(printf '\x7d\x7d')
    PATTERN="${LB}[A-Z_]*${RB}"
    HITS=$(grep -rn "$PATTERN" --include="*.adoc" --include="*.md" --include="*.a2ml" \
           --include="*.scm" --include="*.toml" --include="*.yml" --include="*.yaml" \
           . 2>/dev/null | grep -v 'node_modules\|\.git/' | grep -v 'PLACEHOLDERS.adoc' | grep -v '.machine_readable/self-validating/' || true)
    if [ -n "$HITS" ]; then
        echo ""
    ...
    # Check for template defaults still present (avoid self-match via concatenation)
    TMPL="rsr-template""-repo"
    if grep -q "$TMPL" Justfile 2>/dev/null; then
        echo "⚠ Justfile still references template name — update project name"
        FOUND=1
    fi
...
cookbook:
    ...
    echo "= krl Justfile Cookbook" > "$OUTPUT"
...
man:
    #!/usr/bin/env bash
    mkdir -p docs/man
    cat > docs/man/krl.1 << EOF
    .TH krl 1 "$(date +%Y-%m-%d)" "{{version}}" "krl Manual"
    .SH NAME
    krl \- RSR-compliant project
    .SH SYNOPSIS
    .B just
    [recipe] [args...]
    .SH DESCRIPTION
...
container-init:
    ...
    if [ ! -d "container" ]; then
        ...
        echo "This repo may not have been created from krl template."
        exit 1
    fi
    ...
    read -rp "Service name (e.g. my-api) [krl]: " _SERVICE_NAME
    SERVICE_NAME="${_SERVICE_NAME:-krl}"
...
container-build *args:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman build -t krl:latest -f container/Containerfile .
...
container-push:
    ...
    if [ -f "container/ct-build.sh" ]; then
        ...
        podman push krl:latest
    fi

# Run container interactively (for debugging)
container-run *args:
    podman run --rm -it krl:latest {{args}}
...
doctor:
    @echo "Running diagnostics for krl..."
...
help-me:
    ...
    @echo "  https://github.com/hyperpolymath/krl/issues/new"
    @echo ""
