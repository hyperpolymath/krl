...
= ABI/FFI Standards
NOTE: Template line removed — this is now KRL-specific documentation

# KRL ABI/FFI Documentation
...
│       └── include/
│           └── krl.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── krl.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/krl.h
```
...
int main() {
    void* handle = krl_init();
    if (!handle) return 1;

    int result = krl_process(handle, 42);
    if (result != 0) {
        const char* err = krl_last_error();
...
Compile with:
```bash
gcc -o example example.c -lkrl -L./zig-out/lib
```

### From Idris2

```idris
import KRL.ABI.Foreign
...
#[link(name = "krl")]
extern "C" {
    fn krl_init() -> *mut std::ffi::c_void;
    fn krl_free(handle: *mut std::ffi::c_void);
    fn krl_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}
...
fn main() {
    unsafe {
        ...
        let result = krl_process(handle, 42);
        assert_eq!(result, 0);
...
function init()
    handle = ccall((:krl_init, libkrl), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:krl_process, libkrl), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:krl_free, libkrl), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/abi/Types.idr -o generated/abi/krl.h
   ```
