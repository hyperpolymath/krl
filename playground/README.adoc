...
A local PWA playground for KRL (Knot Resolution Language, pronounced
"`curl`").
...
* *Backend:* KRL source → KRLAdapter.jl (for TangleIR + Skein
persistence)
* *Runtime:* Deno server calling Julia via stdin/stdout or HTTP to a
`+KRLAdapter.jl+`-powered service
* *UI:* ReScript + React SPA with Monaco editor configured for KRL
syntax
* *Execution modes:*
** `+parse+` — show AST
** `+typecheck+` — boundary arity check, port compatibility
** `+compile+` — TangleIR output
** `+invariants+` — compute Jones, Alexander, determinant, signature via
KRLAdapter
...
This playground is blocked on KRL reaching grade E (grammar + 1 smoke
test). See repo-level `+READINESS.md+` for the KRL implementation path.
