...
#
# Containerfile for KRL
# Build: podman build -t krl:latest -f Containerfile .
# Run:   podman run --rm -it krl:latest
...
# Examples:
#   COPY --from=build /build/target/release/krl /usr/local/bin/
#   COPY --from=build /build/_build/prod/rel/krl /app/
#   COPY --from=build /build/zig-out/bin/krl /usr/local/bin/

# Non-root user (chainguard images default to nonroot)
USER nonroot

# TODO: Replace with your entrypoint
# ENTRYPOINT ["/usr/local/bin/krl"]
